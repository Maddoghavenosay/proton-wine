                              ...
                              const struct window_rects *new_rects, struct window_surface *surface)
{
    ...
    else
    {
        ...
        if (wayland_win_data_create_wayland_surface(data, toplevel_surface, owner_surface))
        {
    ...
    if (new_client != data->client_surface)
    {
        ...
        if ((data->client_surface = new_client))
        {
            ...
            if (desktop_mode && toplevel == hwnd && !data->wayland_surface &&
                NtUserIsWindowVisible(hwnd) && wayland_win_data_create_wayland_surface(data, NULL, NULL))
            {
                wayland_win_data_update_wayland_state(data);
                wayland_desktop_report_window(data);
                placed = TRUE;
